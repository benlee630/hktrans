const providers = {
  ...
  ctb: {
    ...
    async resolveAllDirections(route, stopText, routeMeta, app) {
      ...
      for (const dir of ['outbound', 'inbound']) {
        try {
          ...
          if (!stopList.length) {
            app.debug.citybus.push({
              stage: 'route-stop-empty',
              direction: dir,
              route,
          ...
          if (!stopId) {
            app.debug.citybus.push({
              ...
              route,
              usedUrl,
              sample: enrichedStopList.slice(0, 3)
            });
            continue;
          }
    ...
    resolveDestName(direction, routeMeta) {
      ...
      return isOut
        ? (routeMeta?.dest_en || routeMeta?.dest_tc || routeMeta?.dest || '')
        : (routeMeta?.orig_en || routeMeta?.orig_tc || routeMeta?.orig || '');
    },
...
const app = {
  ...
  async fetchJson(url) {
    ...
    try {
      ...
      throw new Error(`Invalid JSON: ${text.slice(0, 350)}`);
    }
  },
  ...
  renderResult() {
    const s = this.state;
    const debugBox = this.config.debug
      ? `<div style="margin-top:10px; padding:10px; border:1px dashed #444; border-radius:8px; font-size:12px; color:#bbb; white-space:pre-wrap;">${this.escapeHtml(JSON.stringify(this.debug.citybus.slice(-8), null, 2))}</div>`
      : '';
    ...
    this.dom.result.innerHTML = `
      ...
      <div class="row" style="margin-top:4px;">
        <span class="small">查詢 key：${this.escapeHtml(this.getRouteKey(s.route))}</span>
      </div>
      <div class="row" style="margin-top:4px;">
