(function () {
  function applyCitybusOnlyPatch(app) {
    ...
    const ctb = {
      ...
      brandName: 'Citybus',
      routeKey(route) {
        return `ctb:${String(route || '').trim().toUpperCase()}`;
      },
      ...
      async fetchRouteMeta(route, appRef) {
        ...
        for (const url of urls) {
          try {
            ...
            const meta = json && json.data ? json.data : (json || {});
            if (meta && Object.keys(meta).length) {
              log({ stage: 'route-meta-ok', route: routeNo, url });
              return meta;
            }
          } catch (err) {
      ...
      async resolveAllDirections(route, stopText, routeMeta, appRef) {
        const routeNo = String(route || '').trim().toUpperCase();
        const routeData = routeMeta && routeMeta.data ? routeMeta.data : (routeMeta || {});
        const found = [];
        ...
        for (const dir of ['outbound', 'inbound']) {
          ...
          if (!stopId) {
            log({ stage: 'no-stopid', route: routeNo, direction: dir, usedUrl });
            continue;
          }
    ...
    const originalGetProvider = app.getProvider ? app.getProvider.bind(app) : null;
    ...
    const originalHandleSearch = app.handleSearch ? app.handleSearch.bind(app) : null;
  ...
  window.applyCitybusOnlyPatch = applyCitybusOnlyPatch;
})();
